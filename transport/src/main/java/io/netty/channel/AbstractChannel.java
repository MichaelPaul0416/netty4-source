...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    protected AbstractChannel(Channel parent) {
        this.parent = parent;
        id = newId();
        unsafe = newUnsafe();//server的话返回NioMessageUnsafe,server建立与client的话，返回NioByteUnsafe,这是个抽象方法，由各自的子类实现
    ...
    public ChannelFuture bind(SocketAddress localAddress, ChannelPromise promise) {
        return pipeline.bind(localAddress, promise);//executor is EventExecutor in bossGroup
    }
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        @Override
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {//各自角色指定的EventLoopGroup中选出来的EventLoop[本质上其实就是个EventExecutor]
            if (eventLoop == null) {
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                doRegister();//最终委托给Nio的SelectableChannel#register(Selector)
                neverRegistered = false;
                registered = true;

                // Ensure we call handlerAdded(...) before we actually notify the promise. This is needed as the
                // user may already fire events through the pipeline in the ChannelFutureListener.
                pipeline.invokeHandlerAddedIfNeeded();//之前设置过PendingHandlerCallback,现在开始回调
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            if (!wasActive && isActive()) {//之前不是active但是现在是active
                invokeLater(new Runnable() {
                    @Override
                    public void run() {
                        pipeline.fireChannelActive();//bind完了之后调用Active
                    }
                });
            }
        ...
        public final void write(Object msg, ChannelPromise promise) {
            ...
            int size;
            try {
                msg = filterOutboundMessage(msg);//client的话直接返回，server的话看具体的实现类，NioServerSocketChannel不支持这个
