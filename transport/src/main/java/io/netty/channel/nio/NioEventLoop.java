...
public final class NioEventLoop extends SingleThreadEventLoop {
    ...
    private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {//server调用时，ch一般是channel方法注册的class实例，此处为NioServerSocketChannel
        ...
        try {
            ...
            if ((readyOps & SelectionKey.OP_CONNECT) != 0) {//说明本身的状态已经是Connect了-->SelectKey#isConnectable
                // remove OP_CONNECT as otherwise Selector.select(..) will always return without blocking
                // See https://github.com/netty/netty/issues/924
                int ops = k.interestOps();
                ops &= ~SelectionKey.OP_CONNECT;//返回实际的感兴趣的操作位
                k.interestOps(ops);
            ...
            // to a spin loop
            if ((readyOps & (SelectionKey.OP_READ | SelectionKey.OP_ACCEPT)) != 0 || readyOps == 0) {//可读或可接受 或者 啥都不是
                unsafe.read();
            }
