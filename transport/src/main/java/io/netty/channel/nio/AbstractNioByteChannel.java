...
public abstract class AbstractNioByteChannel extends AbstractNioChannel {
    ...
    protected AbstractNioByteChannel(Channel parent, SelectableChannel ch) {
        super(parent, ch, SelectionKey.OP_READ);//生成一个新的DefaultChannelPipeline.并且初始化完成ChannelHandlerContext列表[包含HeadContext,TailContext],同时生成unsafe
    }
    ...
    protected class NioByteUnsafe extends AbstractNioUnsafe {
        ...
        public final void read() {
            final ChannelConfig config = config();//NioServerSocketChannel中初始化的话，实例化类型为NioServerSocketChannelConfig
            ...
            try {
                do {
                    ...
                    allocHandle.lastBytesRead(doReadBytes(byteBuf));//record total read bytes
                    ...
                    readPending = false;
                    pipeline.fireChannelRead(byteBuf);//此时的bytebuf可能是半包或者有粘包的情况，需要解码器手动处理
                    byteBuf = null;
                } while (allocHandle.continueReading());//一般来说是判断消息是否已经读完整
