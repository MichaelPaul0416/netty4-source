...
abstract class AbstractChannelHandlerContext extends DefaultAttributeMap
    ...
    public ChannelFuture bind(final SocketAddress localAddress, final ChannelPromise promise) {
        ...
        //有点类似AQS中release的时候，从后往前找当前持有lock的node，AQS中这样做的目的是为了去除其他node在等待获取lock的过程中取消了等待
        //但是这里不同的是，这个方法是从tail往前找到第一个ChannelOutboundHandler的Context，而不是最后一个，AQS是找到最后一个
    ...
    private void invokeBind(SocketAddress localAddress, ChannelPromise promise) {
        if (invokeHandler()) {//这个Context绑定的Handler已经加入到Context的双向链表中，或者说是否还是在pending
            try {
                //如果是加入到了链表中或者再等，那么结合findContextOutbound()，当前这个Context绑定的ChannelHandler一定是ChannelOutboundHandler
                ((ChannelOutboundHandler) handler()).bind(this, localAddress, promise);
            } catch (Throwable t) {
                notifyOutboundHandlerException(t, promise);
            }
        } else {
            bind(localAddress, promise);//如果不符合的话，继续往前找ChannelOutboundHandler
        }
    }
    ...
    public ChannelFuture connect(
        ...
        final AbstractChannelHandlerContext next = findContextOutbound();//从TailContext绑定的ChannelHandler往前找，找到最先遇到的ChannelOutboundHandler
        EventExecutor executor = next.executor();
        if (executor.inEventLoop()) {
    ...
    private void invokeConnect(SocketAddress remoteAddress, SocketAddress localAddress, ChannelPromise promise) {
        if (invokeHandler()) {
            try {
                ((ChannelOutboundHandler) handler()).connect(this, remoteAddress, localAddress, promise);//handler()方法获取的就是通过findContextOutbound()方法获取的ChannelOutboundHandler
            } catch (Throwable t) {
                notifyOutboundHandlerException(t, promise);
            }
        } else {
    ...
    private void invokeWrite(Object msg, ChannelPromise promise) {
        ...
        } else {//等待，直到handlerState=ADD_COMPLETE
            write(msg, promise);
        }
    }
    ...
    private AbstractChannelHandlerContext findContextOutbound() {
        AbstractChannelHandlerContext ctx = this;//刚开始进来的时候是TailContext
        do {
            ctx = ctx.prev;
        } while (!ctx.outbound);
        return ctx;
    ...
    private boolean invokeHandler() {
        ...
        return handlerState == ADD_COMPLETE || (!ordered && handlerState == ADD_PENDING);//已经加入到Context的双向链表完成或这个context不是处于加入等待[见PendingHandlerCallback]
    }
